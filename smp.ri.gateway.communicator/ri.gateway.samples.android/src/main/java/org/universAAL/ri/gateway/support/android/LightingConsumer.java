...
public class LightingConsumer extends ContextSubscriber {
	...
	MergedRestriction subjectRestriction2 = MergedRestriction
		.getAllValuesRestrictionWithCardinality(
			ContextEvent.PROP_RDF_SUBJECT,
			LightSource.MY_URI, 1, 1);
	cep.addRestriction(subjectRestriction2);
	
	return new ContextEventPattern[] { cep };
    }

    LightingConsumer(final ModuleContext context) {
    ...
    // given URI
    private static ServiceRequest turnOffRequest(final String lampURI) {
    ...
    // see turnOffRequest
    private static ServiceRequest turnOnRequest(final String lampURI) {
