...
import org.universAAL.ri.gateway.support.home.LampStateListener;

/**
 * @author mtazari
 * 
 */
public class LightingProvider1 extends ServiceCallee implements
	LampStateListener {
    ...
    static final String LAMP_URI_PREFIX = LightingService1.LIGHTING_SERVER_NAMESPACE
	    + "controlledLamp";
    ...
    public LightingProvider1(ModuleContext context) {
	super(context, LightingService1.profiles);
	...
	ContextProvider info = new ContextProvider(
		LightingService1.LIGHTING_SERVER_NAMESPACE
	...
	if (operation
		.startsWith(LightingService1.SERVICE_GET_CONTROLLED_LAMPS))
	    return getControlledLamps();

	Object input = call
		.getInputValue(LightingService1.INPUT_LAMP_URI);
	if (input == null)
	    return null;

	if (operation.startsWith(LightingService1.SERVICE_GET_LAMP_INFO))
	    return getLampInfo(input.toString());

	if (operation.startsWith(LightingService1.SERVICE_TURN_OFF))
	    return turnOff(input.toString());

	if (operation.startsWith(LightingService1.SERVICE_TURN_ON))
