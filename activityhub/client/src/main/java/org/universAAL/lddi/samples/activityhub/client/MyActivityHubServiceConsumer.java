...
public class MyActivityHubServiceConsumer {
	...
	private static final String ACTIVITYHUB_CONSUMER_NAMESPACE = "http://ontology.universAAL.org/ActivityHubConsumer.owl#";

	private static final String OUTPUT_LIST_OF_AHS = ACTIVITYHUB_CONSUMER_NAMESPACE + "controlledActivityHubSensors";
	private static final String OUTPUT_AHS_LOCATION = ACTIVITYHUB_CONSUMER_NAMESPACE + "activityHubSensorLocation";
	private static final String OUTPUT_AHS_MEASUREMENT = ACTIVITYHUB_CONSUMER_NAMESPACE
			+ "activityHubSensorMeasurement";
	...
	public static ArrayList getControlledActivityHubSensors() {
		...
		if (sr.getCallStatus() == CallStatus.succeeded) {
			try {
				...
				/*
				 * Debug test: 2 elements in list: MotionSensor with 1 element
				 * in props
				 ...
				 * } and uri
				 * http://ontology.universAAL.org/ActivityHubServer.owl#
				 * controlledActivityHubDevice0/0/2
				 */

				// exit if no elements
				if (ahsList == null || ahsList.size() == 0) {
					LogUtils.logWarn(Activator.mc, MyActivityHubServiceConsumer.class,
							"getControlledActivityHubSensors", new Object[] { "there are no ActivityHubSensors" },
							null);
					return null;
				}
				LogUtils.logInfo(Activator.mc, MyActivityHubServiceConsumer.class, "getControlledActivityHubSensors",
				...
				while (it.hasNext()) {
					...
					LogUtils.logInfo(Activator.mc, MyActivityHubServiceConsumer.class,
							...
							new Object[] { "resource URI: " + ahs.getURI() + " sensorType: " + ahs.getSensorType() },
							null);
				...
				LogUtils.logError(Activator.mc, MyActivityHubServiceConsumer.class, "getControlledActivityHubSensors",
						new Object[] { "got exception", e.getMessage() }, e);
				return null;
			}
		} else {
			LogUtils.logWarn(Activator.mc, MyActivityHubServiceConsumer.class, "getControlledActivityHubSensors",
				...
				for (Iterator iter = outputs.iterator(); iter.hasNext();) {
					...
					if (po != null) {
						LogUtils.logError(Activator.mc, MyActivityHubServiceConsumer.class,
								"getControlledActivityHubSensors",
								new Object[] { "Error message: ", po.getParameterValue() }, null);
					}
				}
			}
			return null;
	...
	public static List getActivityHubSensorInfo(ActivityHubSensor ahs) {
		...
		LogUtils.logDebug(Activator.mc, MyActivityHubServiceConsumer.class, "getActivityHubSensorInfo",
		...
		if (sr.getCallStatus() == CallStatus.succeeded) {
			try {
				...
				if ((listLocation == null || listLocation.size() == 0)
					...
					LogUtils.logWarn(Activator.mc, MyActivityHubServiceConsumer.class, "getActivityHubSensorInfo",
							new Object[] { "there are no details in the response" }, null);
					return null;
				}
				LogUtils.logDebug(Activator.mc, MyActivityHubServiceConsumer.class, "getActivityHubSensorInfo",
						new Object[] { "there are " + listLocation.size() + " locations ",
								"and " + listMeasurement.size() + " measurements in the response!" },
						null);
				...
				while (it.hasNext()) {
					...
					LogUtils.logDebug(Activator.mc, MyActivityHubServiceConsumer.class, "getActivityHubSensorInfo",
							new Object[] { "resource URI: " + res.getURI(), "; resource Type: " + res.getType() },
							null);
				}
				...
				LogUtils.logError(Activator.mc, MyActivityHubServiceConsumer.class, "getActivityHubSensorInfo",
						new Object[] { "got exception", e.getMessage() }, e);
				return null;
			}
		} else {
	...
	public void getSensors() {
		...
		if (ahs != null && !ahs.isEmpty()) {
			...
			for (Iterator it = ahs.iterator(); it.hasNext(); i++) {
				...
				/*
				 * Debug test: singleAHS is of Type MotionSensor,
				 ...
				 * controlledActivityHubDevice1/1/1
				 * http://ontology.universAAL.org/ActivityHubServer.owl#
				 * controlledActivityHubDevice0/0/2 ahsTypes[i] =
				 * (singleAHS).getType();
				 ...
				 * ahsNames[i], " class Name: " +
				 * singleAHS.getClass().getName(), " package: " +
				 * singleAHS.getClass().getPackage(), " class simpleName: " +
				 * singleAHS.getClass().getSimpleName() }, null);
				 */
				...
				if (this.ahc != null && singleAHS != null)
					...
					LogUtils.logError(Activator.mc, MyActivityHubServiceConsumer.class, "getSensors",
	 ...
	 *            is the resourceURI
	 * @param deviceType
	 *            is the type-id for the ActivityHubSensor
	 */
	public void getDeviceInfo(String sensorURI, int deviceType) {
		...
		/*
		 * Debug test LogUtils.logWarn(Activator.mc,
		 * MyActivityHubServiceConsumer.class, "getDeviceInfo", new Object[] {
		 * "SensorType: " + ahs.getType(), "ClassURI: " + ahs.getClassURI(),
		 * "OntClassInfo: " + ahs.getOntClassInfo() // All the same URIs; e.g.
		...
		if (l != null && !l.isEmpty()) {
			...
			if (this.ahc != null)
				...
				LogUtils.logError(Activator.mc, MyActivityHubServiceConsumer.class, "getDeviceInfo",
						new Object[] { "No GUI object found! Discard response!" }, null);
		} else {
			LogUtils.logWarn(Activator.mc, MyActivityHubServiceConsumer.class, "getDeviceInfo",
	...
	/**
	 * Create ontological service request input is one specific sensor object 2
	 * outputs: location and last sensor event
	 */
	private static ServiceRequest getSensorInfoRequest(ActivityHubSensor ahs) {
		...
		// sensorURI);
		// couldn't get this to work on the server side since sensorURI is a
		// String
		...
		getActivityHubSensorInfo.addRequiredOutput(
				// this is OUR unique ID with which we can later retrieve the
				// returned value
				OUTPUT_AHS_MEASUREMENT, new String[] { ActivityHub.PROP_CONTROLS, ActivityHubSensor.PROP_LASTEVENT });
