...
public class LampStatePublisher {
    ...
    public static final String SLEEPING_ROOM_LIGHTING = NAMESPACE
	    + "sleepingRoomLampState";
    public static final String LIVING_ROOM_LIGHTING = NAMESPACE
	    + "livingRoomLampState";
    public static final String BATHROOM_LIGHTING = NAMESPACE
	    + "bathroomLampState";
    public static final String KITCHEN_LIGHTING = NAMESPACE
	    + "kitchenLampState";
    public static final String HOBBY_ROOM_LIGHTING = NAMESPACE
	    + "hobbyRoomLampState";
	...
	Image onImage = Toolkit.getDefaultToolkit().createImage(
		this.getClass().getClassLoader().getResource("lightOn.jpg"));

	this.offImageIcon = new ImageIcon(offImage);
	this.onImageIcon = new ImageIcon(onImage);

	// Insert house light sources to hashmap
	this.lampMap.insertLightSource(SLEEPING_ROOM_LIGHTING, new LightSource(
		SLEEPING_ROOM_LIGHTING));
	this.lampMap.insertLightSource(LIVING_ROOM_LIGHTING, new LightSource(
		LIVING_ROOM_LIGHTING));
	this.lampMap.insertLightSource(BATHROOM_LIGHTING, new LightSource(
		BATHROOM_LIGHTING));
	this.lampMap.insertLightSource(KITCHEN_LIGHTING, new LightSource(
		KITCHEN_LIGHTING));
	this.lampMap.insertLightSource(HOBBY_ROOM_LIGHTING, new LightSource(
		HOBBY_ROOM_LIGHTING));

	LampStatePublisher.myModuleContext = context;

	ContextProvider myContextProvider = new ContextProvider(NAMESPACE
		+ "lampStateContextProvider");
	...
	ContextEvent myContextEvent = new ContextEvent(lightSource,
		LightSource.PROP_SOURCE_BRIGHTNESS);
