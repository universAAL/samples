...
public class CalleeExample extends ServiceCallee {
	...
	private static ServiceProfile[] realizedServices() {
		...
		// GET LAMP INFO profile: Input<light:?> Output<Brightness>
		// Output<Location>
		...
		prof1.put(Path.at(DeviceService.PROP_CONTROLS).to(LightController.PROP_HAS_VALUE),
				Arg.out(TypeMapper.getDatatypeURI(Integer.class)), OUTPUT_LAMP_BRIGHTNESS);
		prof1.put(Path.at(DeviceService.PROP_CONTROLS).to(LightController.PROP_PHYSICAL_LOCATION),
				Arg.out(Location.MY_URI), OUTPUT_LAMP_LOCATION);
		...
		prof2.put(Path.at(DeviceService.PROP_CONTROLS).to(LightController.PROP_HAS_VALUE),
				Arg.change(TypeMapper.getDatatypeURI(Integer.class)), INPUT_BRIGHTNESS);
		...
		prof3.put(Path.at(DeviceService.PROP_CONTROLS).to(LightController.PROP_HAS_VALUE),
				Arg.change(Integer.valueOf(100)), null);
		...
		prof4.put(Path.at(DeviceService.PROP_CONTROLS).to(LightController.PROP_HAS_VALUE),
				Arg.change(Integer.valueOf(0)), null);
	...
	public ServiceResponse handleCall(ServiceCall call) {
		...
		if (operation.startsWith(SERVICE_GET_CONTROLLED_LAMPS)) {
			// Return all lights (use clone because otherwise they get modified
			// in the process)
		...
		if (input == null) {
			// The rest of profiles use a light input. If it's not there,
			// failure!
		...
		if (operation.startsWith(SERVICE_GET_LAMP_INFO)) {
			...
			ServiceResponse sr = new ServiceResponse(CallStatus.succeeded);
			sr.addOutput(new ProcessOutput(OUTPUT_LAMP_BRIGHTNESS,
					Integer.valueOf(Activator.myLights.get(index).getValue())));
		...
		if (operation.startsWith(SERVICE_TURN_DIM)) {
			...
			if (Activator.myLights.get(index).getValue() != bright) {
				...
				Activator.publisher
						.publish(new ContextEvent(Activator.myLights.get(index), LightController.PROP_HAS_VALUE));
			}
