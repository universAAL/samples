...
public class ProvidedService extends Lighting {
	...
	// of the object
	OntologyManagement.getInstance().register(Activator.moduleContext,
	...
	// services that we want to make
	// available. We start with some "class-level restrictions" that are
	// inherent to the underlying
...
//	addRestriction(
//		(MergedRestriction) Lighting.getClassRestrictionsOnProperty(
...
//		serverLightingRestrictions);
	...
	// sources is ElectricLight.lightBulb
	// meaning that light sources controlled by this class of services are
	// all light bulbs
	...
	addRestriction(MergedRestriction.getAllValuesRestrictionWithCardinality(
		...
		1), ppBrightness, serverLightingRestrictions);
	...
	ProvidedService getControlledLamps = new ProvidedService(
	...
	ProvidedService getLampInfo = new ProvidedService(SERVICE_GET_LAMP_INFO);
	...
	// objects addressed by
	// 'ppControls' only those are selected that have the same identity as
	// the value passed
	...
	// about
	// the brightness (parameter #5) of the light bulb in the scope (cf. the
	// input parameter);
	// this info will be a single (parameters #3 & #4) number of type
	// integer (parameter #2)
	// that is assigned to an output parameter identifiable by the given URI
	// (parameter 1)
	...
	// input parameter);
	// this info will be a single (parameters #3 & #4) object of type
	...
	ProvidedService turnOff = new ProvidedService(SERVICE_TURN_OFF);
	...
	// (parameter #1)
	// of the selected light bulb (cf. the input parameter) to 0 (parameter
	// #2)
	...
	ProvidedService turnOn = new ProvidedService(SERVICE_TURN_ON);
	// We need an input parameter identical with the previous one
	turnOn.addFilteringInput(INPUT_LAMP_URI, LightSource.MY_URI, 1, 1,
		ppControls);
	// but the result of using this service is the change of the brightness
	// (parameter #1)
	// of the selected light bulb (cf. the input parameter) to 100
	// (parameter #2)
