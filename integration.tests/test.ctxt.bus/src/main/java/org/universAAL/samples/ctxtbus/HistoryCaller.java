...
public class HistoryCaller {
	...
	public int callGetEvents(org.universAAL.ontology.che.ContextEvent matchEvent, long tstFrom, long tstTo) {
		int count = 0;
	...
	private ServiceRequest getGetEventsRequest(org.universAAL.ontology.che.ContextEvent matchEvent, long tstFrom,
			long tstTo) {
		ServiceRequest getEvents = new ServiceRequest(new ContextHistoryService(null), null);
		...
		if (tstFrom > 0) {
			MergedRestriction tstr1 = MergedRestriction
					.getFixedValueRestriction(ContextHistoryService.PROP_TIMESTAMP_FROM, new Long(tstFrom));
		...
		if (tstTo > 0) {
			MergedRestriction tstr2 = MergedRestriction
					.getFixedValueRestriction(ContextHistoryService.PROP_TIMESTAMP_TO, new Long(tstTo));
	...
	public String callDoSPARQL(String query) {
		...
		if (response.getCallStatus() == CallStatus.succeeded) {
			try {
				...
				// getReturnValue(response.getOutputs(),
			...
			log.info("History Client - status of doSparqlQuery(): " + response.getCallStatus());
		return "";
	}
