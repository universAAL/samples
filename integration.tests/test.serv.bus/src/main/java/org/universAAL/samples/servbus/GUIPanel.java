...
public class GUIPanel extends javax.swing.JFrame {
	...
	private static final String OUTPUT_LIST_OF_LAMPS = LIGHTING_CLIENT_NAMESPACE + "listOfLamps";
	private static final String OUTPUT_LAMP_LOCATION = LIGHTING_CLIENT_NAMESPACE + "lampLocation";
	...
	private void initComponents() {
		...
		combo1p1 = new javax.swing.JComboBox(
				new Object[] { "Get Lamps", "Get Lamp Info", "Turn On", "Turn Off", "Set 50%" });
	...
	private ServiceRequest getGETLAMPSrequest() {
		ServiceRequest listLamps = new ServiceRequest(Lighting.OWLS_NAMESPACE_PREFIX + "requestURI", new Lighting(null),
				null);
		listLamps.addSimpleOutputBinding(new ProcessOutput(OUTPUT_LIST_OF_LAMPS),
				new PropertyPath(null, true, new String[] { Lighting.PROP_CONTROLS }).getThePath());
		return listLamps;
	}
	...
	private ServiceRequest getGETLAMPINFOrequest() {
		...
		getLampLocation.getRequestedService().addInstanceLevelRestriction(
				MergedRestriction.getFixedValueRestriction(Lighting.PROP_CONTROLS, new LightSource(SAMPLE_LAMP_URI)),
				new String[] { Lighting.PROP_CONTROLS });
		getLampLocation
				.addSimpleOutputBinding(new ProcessOutput(OUTPUT_LAMP_LOCATION),
	...
	private ServiceRequest getSETreqest(int brightness) {
		ServiceRequest setLampBrightness = new ServiceRequest(new Lighting(), null);
		setLampBrightness.getRequestedService().addInstanceLevelRestriction(
				MergedRestriction.getFixedValueRestriction(Lighting.PROP_CONTROLS, new LightSource(SAMPLE_LAMP_URI)),
				new String[] { Lighting.PROP_CONTROLS });
		setLampBrightness.addChangeEffect(
				new PropertyPath(null, true,
