...
public class ContextEventRecorder extends ContextSubscriber {
	...
	private static ContextEventPattern[] createInitialSubscription() {
		...
		contextEventPattern.addRestriction(
				MergedRestriction.getAllValuesRestriction(ContextEvent.PROP_RDF_SUBJECT, ManagedIndividual.MY_URI));
	...
	public void handleContextEvent(ContextEvent event) {
		...
		if (event.getRDFPredicate() != null) {
			...
			if (event.getRDFObject() instanceof ManagedIndividual) {
				String uri = ((ManagedIndividual) event.getRDFObject()).getURI();
	...
	public boolean saveData() {
		return (saveArray(ContextEventRecorder.URI_FILENAME, this.recordedURIs)
				&& saveArray(ContextEventRecorder.TYPE_URI_FILENAME, this.recordedTypeURIs)
				&& saveArray(ContextEventRecorder.PREDICATE_FILENAME, this.recordedPredicates)
				&& recordedEvents.saveElements());
	}
	 ...
	 */
	private <O extends Object> boolean saveArray(String filename, ArrayList<O> list) {
		try {
			...
			for (O object : list)
				bw.write(object.toString() + System.getProperty("line.separator"));
			bw.close();
