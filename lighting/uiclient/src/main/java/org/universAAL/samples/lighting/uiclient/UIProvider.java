...
public class UIProvider extends UICaller {
	...
	static final String MY_UI_NAMESPACE = SharedResources.CLIENT_LIGHTING_UI_NAMESPACE + "UIProvider#";
	...
	static final PropertyPath PROP_PATH_SCALE_VALUE = new PropertyPath(null, false,
			new String[] { USER_INPUT_SELECTED_LAMP, LightSource.PROP_SOURCE_BRIGHTNESS });
	static final PropertyPath PROP_PATH_SELECTED_LAMP = new PropertyPath(null, false,
			new String[] { USER_INPUT_SELECTED_LAMP });
	...
	protected UIProvider(ModuleContext context) {
		...
		for (int i = 0; i < devices.length; i++) {
			...
			if (label == null) {
				...
				String type = StringUtils.deriveLabel((lt == null) ? devices[i].getClassURI() : lt.getURI());
	...
	private Form initMainDialog() {
		...
		Select1 select = new Select1(f.getIOControls(),
				new Label("Lamps", null), PROP_PATH_SELECTED_LAMP, MergedRestriction
						.getAllValuesRestrictionWithCardinality(USER_INPUT_SELECTED_LAMP, LightSource.MY_URI, 1, 1),
				devices[0]);
		...
		InputField in = new InputField(f.getIOControls(), new Label("Scale Value", null), PROP_PATH_SCALE_VALUE, null,
				null);
		in.setAlertString("Expected: a number between 0 and 100!");

		// add the possible operations while specifying the mandatory input
		// needed form them
		new Submit(f.getSubmits(), new Label("On", null), SUBMISSION_ON).addMandatoryInput(select);
		new Submit(f.getSubmits(), new Label("Off", null), SUBMISSION_OFF).addMandatoryInput(select);
		Submit s = new Submit(f.getSubmits(), new Label("Scale", null), SUBMISSION_SCALE);
	...
	void startMainDialog() {
		...
		UIRequest out = new UIRequest(SharedResources.testUser, mainDialog, LevelRating.middle, Locale.ENGLISH,
