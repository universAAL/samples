...
public class LightingProvider extends ServiceCallee implements LampStateListener {
	...
	private static LightSource[] getAllLightSources(MyLighting theServer) {
		...
		for (int i = 0; i < lamps.length; i++)
			result[i] = new LightSource(
					...
					new Room(constructLocationURIfromLocalID(theServer.getLampLocation(lamps[i]))));
		return result;
	}
	...
	public static ContextEventPattern[] providedEvents(MyLighting theServer) {
		...
		MergedRestriction predicateRestriction = MergedRestriction
				.getFixedValueRestriction(ContextEvent.PROP_RDF_PREDICATE, LightSource.PROP_SOURCE_BRIGHTNESS);
		...
		MergedRestriction objectRestriction = MergedRestriction.getAllValuesRestrictionWithCardinality(
				ContextEvent.PROP_RDF_OBJECT, new Enumeration(new Integer[] { new Integer(0), new Integer(100) }), 1,
				1);
		...
		MergedRestriction subjectRestriction = MergedRestriction
				.getAllValuesRestrictionWithCardinality(ContextEvent.PROP_RDF_SUBJECT, new Enumeration(myLights), 1, 1);
		...
		xsection.addType(
				MergedRestriction.getFixedValueRestriction(LightSource.PROP_HAS_TYPE, ElectricLight.lightBulb));
		xsection.addType(MergedRestriction.getAllValuesRestrictionWithCardinality(LightSource.PROP_PHYSICAL_LOCATION,
				Room.MY_URI, 1, 1));
	...
	LightingProvider(ModuleContext context) {
		...
		try {
			...
			LogUtils.logInfo(Activator.mc, LightingProvider.class, "LightingProvider",
					new Object[] { "server activates GUI-off mode because of no screen access" }, null);
		}
	}
	...
	private ServiceResponse getControlledLamps() {
		...
		for (int i = 0; i < lamps.length; i++) {
			LightSource ls = new LightSource(constructLampURIfromLocalID(lamps[i]));
	...
	private ServiceResponse getLampInfo(String lampURI) {
		try {
			...
			sr.addOutput(new ProcessOutput(ProvidedLightingService.OUTPUT_LAMP_BRIGHTNESS, new Integer(state)));
			// create and add a ProcessOutput-Event that binds the output URI to
	...
	public void lampStateChanged(int lampID, String loc, boolean isOn) {
		...
		LogUtils.logInfo(Activator.mc, LightingProvider.class, "lampStateChanged",
